...
export default function ProductDetails() {
  ...
  const { mutate: addTowhishlist, status: AddToWhislistStatus } =
    useAddToWhislist();
  const { mutate: removeFromWhishlist, status: removeFromWhislistStatus } =
    useDeleteWishlist();
  ...
  function handleMainImageClick(img) {
    setIsOpenImage(img)(isOpenImage);
  }
        ...
        <>
          <div className="flex xs:flex-col md:flex-row  gap-6  pb-6 ">
              ...
              <div className="flex md:self-center xs:flex-row  md:flex-col gap-3">
                {images.map((img, i) => (
                  <img
                    ...
                    className="border shadow-sm rounded-md w-full max-h-[100px] object-cover"
                    src={img}
                    alt=""
                  />
                ))}
            ...
            <div className="flex flex-col xs:w-full md:w-1/2  justify-between">
              <div>
                <h2 className="mb-1 text-4xl font-semibold text-red-700">
                  {title}
                </h2>
                <Link
                  to={`/shop?category[in]=${categoryId}`}
                  className="font-semiboldtext-red-700  underline"
                >
                  {name}
                </Link>
                <p className="my-1">
              ...
              <div>
                <p className="text-red-700 font-semibold my-2">
                  {formatPrice(price)}
                </p>
              ...
              <div className="flex gap-3 my-6 items-center ">
                ...
                {isInCart ? (
                  <Button
                    isLoading={delStauts === "pending"}
                    onclick={() => {
                      deleteItem(_id);
                    }}
                    textContent={"Already In your Cart (Remove)"}
                    styles={"w-full bg-title text-white  "}
                  />
                ) : (
                  <Button
                    ...
                    styles={"w-full bg-primary text-white "}
                  />
                )}

                <div className="border shadow-sm p-2 rounded-lg ">
                  {AddToWhislistStatus === "pending" ||
                  removeFromWhislistStatus === "pending" ? (
                    <Oval color="red" width={20} height={20} />
                  ) : isInWishlist ? (
                    <MdFavorite
                      cursor={"pointer"}
                      onClick={() => removeFromWhishlist(productId)}
                      color="red"
                      fontSize={20}
                    />
                  ) : (
                    <MdFavoriteBorder
                      cursor={"pointer"}
                      onClick={() => addTowhishlist({ productId: productId })}
                      color="red"
                      fontSize={20}
                      className="text-red"
                    />
          ...
          {!isLoadingRelated && (
            <ProductsRow items={items.data} role="slider" />
          )}
        </>
      )}
    </div>
