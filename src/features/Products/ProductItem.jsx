...
export default function ProductItem({
  ...
  const { mutate: addToWishList, status: addToWishListStatus } =
    useAddToWhislist();
  const { mutate: deleteFromWishlist, status: deleteFromWishlistStatus } =
    useDeleteWishlist();
  ...
  useEffect(() => {
    if (!cart) {
      ...
      setIsInCart(() =>
        cart.data.products.some((item) => item.product._id === _id)
      );
    }
  }, [cart, _id]);
      ...
      <div className="pt-3 px-2 pb-3 bg-primary-2 rounded-lg  ">
        ...
        {isInCart ? (
          <Button
            ...
            textContent={"Already Added (Remove)"}
          ...
          <Button
            ...
            styles={
              "w-full bg-primary dark:bg-primary-3 dark:border dark:border-white text-white "
            }
          />
        )}
      </div>
