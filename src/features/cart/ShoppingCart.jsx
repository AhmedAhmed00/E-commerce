...
export default function Cart() {
  ...
  const {
    cart: { numOfCartItems, data: { products } = {} } = {},
    isLoading,
    isError,
  } = useCart();
  ...
  return (
    <>
      {isLoading ? (
        ...
        <div className="container py-6 text-2xl">You have no cart yet</div>
      ) : (
        <div className="py-6 relative container">
          {confirmShow && (
            <ConfirmDelettion
              handleDeletion={() => handleClearCart()}
              ...
              <div className="flex  xs:flex-col lg:flex-row justify-between items-start gap-6">
                ...
                <PayDetails>
                  <Button
                    onclick={handlePay}
                    styles={
                      "py-1.5 w-full mt-4 mb-1 font-bold bg-sky text-white "
                    }
                    textContent={"Proced to Checkout"}
                  />
                  <Button
                    onclick={() => setConfirmShow((show) => !show)}
                    styles="py-1.5 w-full mt-0  bg-red-700 text-white "
